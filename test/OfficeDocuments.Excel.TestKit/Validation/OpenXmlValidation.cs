...
namespace OfficeDocuments.Excel.TestKit.Validation;

/// <summary>
/// Schema-validation gate for generated workbooks.
/// </summary>
/// <remarks>
/// A test that only round-trips a document through this library proves self-consistency, not
/// validity: the file can still violate the OOXML schema and be repaired or rejected by Excel.
/// Every test that produces a complete document should end with an <see cref="AssertValid(string)"/>
/// call so that schema-order and relationship defects fail the build instead of the user's Excel.
/// </remarks>
public static class OpenXmlValidation
{
    ...
    /// <summary>
    /// Asserts that the workbook stored at <paramref name="filePath"/> is schema-valid.
    /// </summary>
    /// <param name="filePath">Path of the workbook to validate.</param>
    ...
    /// <summary>
    /// Asserts that the workbook held in <paramref name="stream"/> is schema-valid.
    /// The stream position is restored before returning.
    /// </summary>
    /// <param name="stream">Stream holding the workbook package.</param>
    /// <param name="inheritedDefects">
    /// Substrings of validation-error descriptions to tolerate because they originate in a
    /// pre-existing input document rather than in anything this library produced. Real Excel
    /// files in the wild are not always schema-clean, so a test that opens a foreign workbook
    /// must be able to say "this defect came in with the input" without switching the gate off.
    /// </param>
    ...
    /// </summary>
    public static void AssertValid(SpreadsheetDocument document)
    {
        AssertValid(document, "<document>", []);
    }

    private static void AssertValid(SpreadsheetDocument document, string source, string[] inheritedDefects)
    {
